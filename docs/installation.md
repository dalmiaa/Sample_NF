# dalmiaa/Sample_NF Installation

To start using the dalmiaa/Sample_NF pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `dalmiaa/Sample_NF` is specified as the pipeline name.
...
```bash
wget https://github.com/dalmiaa/Sample_NF/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/dalmiaa-sample_nf-master
```
